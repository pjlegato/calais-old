module Calais
  class Client
    ...
    attr_accessor :reltag_baseurl, :relevance_score, :omit_outputting_original_text
    ...
    private
      def params_xml
        ...
        omit_outputting_original_text = @omit_outputting_original_text ? "true" : "false"

        xml  = %[<c:params xmlns:c="http://s.opencalais.com/1/pred/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">]
        xml += %[<c:processingDirectives c:contentType="#{content_type}" c:outputFormat="#{output_format}" c:reltagBaseURL="#{reltag_baseurl}" ] +
          %[c:omitOutputtingOriginalText="#{omit_outputting_original_text}" c:calculateRelevanceScore="#{relevance_score}"></c:processingDirectives>]
